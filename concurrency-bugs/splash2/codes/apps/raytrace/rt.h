...
typedef struct	gmem
	...
	int (start);			/* Barrier for startup sync.	     */
	int (pidlock);		/* Lock to increment pid.	     */
	int (ridlock);		/* Lock to increment rid.	     */
	int (memlock);		/* Lock for memory manager.	     */
	int (wplock);	/* Locks for shared work pools.      */
    UINT par_start_time;
