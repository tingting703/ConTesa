             ...
             unsigned int part_str_size)
{
  ...
  if (num_partitions)
  {
    log_text(LOG_NOTICE, "Creating table %s with %d partitions in table",
...
void *oltp_cmd_prepare_one_table(void *arg)
{
  ...
  char           *partition_str;
  unsigned int   partition_str_size;
  ...
  snprintf(query, query_len,
           ...
           driver_caps.unsigned_int ? "UNSIGNED" : "",
           args.secondary ? "KEY xid (id) USING BTREE" : "PRIMARY KEY (id)",
...
void oltp_print_mode(void)
{
  ...
  if (args.num_partitions)
    log_text(LOG_NOTICE, "Number of partitions per table = %d",
             args.num_partitions);
...
sb_request_t get_request_complex(int tid)
{
  ...
  for(i = 0; i < args.point_selects; i++)
  {
    ...
    if (args.use_in_statement)
    {
      ...
      if ((key_id + 10) >= args.table_size)
        key_id -= 10;
      query->u.in_point_query.id1 = key_id + 0;
      query->u.in_point_query.id2 = key_id + 1;
      query->u.in_point_query.id3 = key_id + 2;
      query->u.in_point_query.id4 = key_id + 3;
      query->u.in_point_query.id5 = key_id + 4;
      query->u.in_point_query.id6 = key_id + 5;
      query->u.in_point_query.id7 = key_id + 6;
      query->u.in_point_query.id8 = key_id + 7;
      query->u.in_point_query.id9 = key_id + 8;
      query->u.in_point_query.id10 = key_id + 9;
      query->nrows = 10;
      SB_LIST_ADD_TAIL(&query->listitem, sql_req->queries);
                         ...
                         char *table_name)
{
  ...
  /* Prepare the IN-statement retrieving 10 rows by PK */
  snprintf(query, MAX_QUERY_LEN, "SELECT c from %s where id in (?,?,?,?,?,?,?,?,?,?)",
           table_name);
  set->in_point = db_prepare(conn, query);
  if (set->in_point == NULL)
    return 1;
  binds[0].type = DB_TYPE_INT;
  binds[0].buffer = &bufs->in_point.id1;
  binds[0].is_null = 0;
  binds[0].data_len = 0;

  binds[1].type = DB_TYPE_INT;
  binds[1].buffer = &bufs->in_point.id2;
  binds[1].is_null = 0;
  binds[1].data_len = 0;

  binds[2].type = DB_TYPE_INT;
  binds[2].buffer = &bufs->in_point.id3;
  binds[2].is_null = 0;
  binds[2].data_len = 0;

  binds[3].type = DB_TYPE_INT;
  binds[3].buffer = &bufs->in_point.id4;
  binds[3].is_null = 0;
  binds[3].data_len = 0;

  binds[4].type = DB_TYPE_INT;
  binds[4].buffer = &bufs->in_point.id5;
  binds[4].is_null = 0;
  binds[4].data_len = 0;

  binds[5].type = DB_TYPE_INT;
  binds[5].buffer = &bufs->in_point.id6;
  binds[5].is_null = 0;
  binds[5].data_len = 0;

  binds[6].type = DB_TYPE_INT;
  binds[6].buffer = &bufs->in_point.id7;
  binds[6].is_null = 0;
  binds[6].data_len = 0;

  binds[7].type = DB_TYPE_INT;
  binds[7].buffer = &bufs->in_point.id8;
  binds[7].is_null = 0;
  binds[7].data_len = 0;

  binds[8].type = DB_TYPE_INT;
  binds[8].buffer = &bufs->in_point.id9;
  binds[8].is_null = 0;
  binds[8].data_len = 0;

  binds[9].type = DB_TYPE_INT;
  binds[9].buffer = &bufs->in_point.id10;
  binds[9].is_null = 0;
  binds[9].data_len = 0;

  if (db_bind_param(set->in_point, binds, 10))
    return 1;
                                 ...
                                 unsigned int thread_id)
{
  ...
  switch (query->type) {
    ...
    case SB_SQL_QUERY_IN_POINT:
      ...
      buf->in_point.id1 = query->u.in_point_query.id1;
      buf->in_point.id2 = query->u.in_point_query.id2;
      buf->in_point.id3 = query->u.in_point_query.id3;
      buf->in_point.id4 = query->u.in_point_query.id4;
      buf->in_point.id5 = query->u.in_point_query.id5;
      buf->in_point.id6 = query->u.in_point_query.id6;
      buf->in_point.id7 = query->u.in_point_query.id7;
      buf->in_point.id8 = query->u.in_point_query.id8;
      buf->in_point.id9 = query->u.in_point_query.id9;
      buf->in_point.id10 = query->u.in_point_query.id10;
      break;
