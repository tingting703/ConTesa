...
namespace klee {
  ...
  public:
    ...
    protected:
        ...
        //symbiosis
        std::set<llvm::Function*> visitedFuncs; //** Nuno: set used to mark the functions containing the events of a given tyu trace
    ...
    class TyuReplaySearcher : public GeneralReplaySearcher {

    public:
        TyuReplaySearcher(Executor &executor);
	    ~TyuReplaySearcher();

        void generateNewShortDistance();
        void findNextTarget();
        void expandTraceFile(); //** Nuno: complete the tyu trace with events referring to the function call seq
