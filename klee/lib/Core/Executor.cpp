...
void printDebug_executor(Instruction *inst){
...
void Executor::initializeGlobals(ExecutionState &state) {
  ...
  for (Module::const_global_iterator i = m->global_begin(),
    ...
    if (i->isDeclaration()) {
      ...
      if (size) {
        void *addr;
        if (i->getName() == "__dso_handle" || i->getName() == "PC" ||
        		i->getName() == "BC"|| i->getName() == "UP") {
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  ...
  if (res==Solver::True) {
    ...
    //** Nuno: uptdate state ids in order to store correctly the symb info
...
void Executor::run(ExecutionState &initialState) {
  ...
  while (!states.empty() && !haltExecution) {
    ...
    //wei: terminates the executor if searcher asks to do so.
	/*if (searcher->empty()){
	...
	/* if(((TyuReplaySearcher*)searcher)->getTarget)
	             {
	          //Nuno: terminate current state but still allow other states to proceed
	          ((TyuReplaySearcher*)searcher)->getTarget = false;
	          std::cerr << "execution halting by searcher\n";
